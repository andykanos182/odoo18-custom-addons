...
from ..const import DOKU_API_URLS, SUPPORTED_CURRENCIES, SUPPORTED_COUNTRIES
...
class PaymentProvider(models.Model):
    ...
    doku_client_id = fields.Char(
        string="DOKU Client ID",
        help="The Client ID provided by DOKU. "
             "Found in DOKU Dashboard > Integration > Credentials.",
        required_if_provider='doku',
        groups='base.group_system',
    )

    doku_secret_key = fields.Char(
        string="DOKU Secret Key",
        help="The Secret Key provided by DOKU. "
    ...
    doku_merchant_code = fields.Char(
        string="DOKU Merchant Code",
        help="Your unique Merchant Code (Mall ID) provided by DOKU.",
        required_if_provider='doku',
        groups='base.group_system',
    )
    ...
    doku_environment = fields.Selection(
        ...
        default='sandbox',
        required_if_provider='doku',
        help="Sandbox: Use for testing with simulated transactions.\n"
    ...
    def _get_default_payment_method_codes(self):
        """Return the default payment method codes for DOKU provider."""
    ...
    # ONCHANGE
    # ==========================================
    @api.onchange('doku_environment')
    def _onchange_doku_environment(self):
        """Auto-update state based on environment selection."""
        if self.code == 'doku':
            if self.doku_environment == 'sandbox':
                self.state = 'test'
            elif self.doku_environment == 'production':
                if self.state == 'test':
                    return {
                        'warning': {
                            'title': _("Environment Changed"),
                            'message': _(
                                "You've selected PRODUCTION environment. "
                                "Please ensure your production credentials are correct "
                                "and change the state to 'Enabled' when ready to go live."
                            ),
                        }
                    }
